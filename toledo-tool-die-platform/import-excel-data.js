...
// Configure Supabase (these will need to be updated with actual credentials)
...
class ToledoDataImporter {
  ...
  async importShiftUpdates() {
    ...
    console.log(`✅ Imported ${files.length} Shift Update files`);
  }
    ...
    try {
      ...
      if (!partData) {
        const { data: newPart } = await supabase
          .from('parts')
          .insert({ part_number: partNumber, part_name: 'Unknown Part' })
          .select()
          .single();
        partData = newPart;
      }
